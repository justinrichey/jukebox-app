...
db.collection('rooms').doc(getPin()).collection('queue')
    ...
    querySnapshot.docChanges().forEach(change => {
        ...
        if (change.type == 'added') {
            ...
            if (!querySnapshot.empty && !isFirstOnQueue) {
                ...
                //Changing the html
                tableSongName.innerHTML = `<p>${change.doc.data().SONG_NAME}</p>`;
                tableSongThumbnail.innerHTML = `<img src=${change.doc.data().THUMBNAIL_URL}>`;
    ...
    if (!querySnapshot.empty) {
        ...
        if (document.getElementById(querySnapshot.docs[0].id)) {
            deleteRow(querySnapshot.docs[0].id);
        }
    } else {
